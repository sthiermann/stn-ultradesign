...
This was a source-and-scenario review with surrounding project context, not a blinded cold start. It did not render an application, execute the scenario against a backend, prove full feature parity or measure usability. The new messaging method and walkthrough template still need broader application trials. The research archive and external source catalogue have been removed from the public package; project-local research remains part of the operating method. Earlier results below retain their original scope and do not retroactively validate this release.

## Version 0.2.0 — historical checks
...
The purpose-first, navigation/materials and web implementation references were expanded using primary documentation accessed on September 16, 2026, for the requested project directions. The later neutrality revision separates these dated investigations from the operational method: project-specific research must follow the user's selected language. The brand-discovery module received an independent read-only review for selective inheritance, source authority, existing user decisions, asset provenance, accessible variants and approval scope. No material contradiction was found in that review. These are research and instruction checks; they do not establish successful application of every new rule to a complete product.

The component-state and material modules extend the method with state composition, control-pattern selection and brand-bound materials. These additions were source-reviewed and linked into the skill and design contract. They have not yet completed an independent application evaluation across all component families.
...
## Fresh interaction and preservation evaluation — 2026-09-16

A fresh agent applied the 0.3.2 instructions to an original synthetic laboratory-monitoring baseline and a deficient concept. It received the task, raw inputs and skill resources, without the intended answer, earlier results or private project feedback. The [independent review](../../evals/interaction-review.md) distinguishes commands, value selection, independent display controls, object editing and messenger-local tasks. It identifies missing concurrent data series, event-stack lifecycle, actual role records and complete required pages, and declines full-scope approval. It also avoids inventing an aggregation formula or treating source inspection as a security test. The [snapshot](../../evals/interaction-snapshot.json) identifies the inputs and skill resources.

The coordinating review compared those findings and proposed contracts with the raw baseline and candidate. The report addresses the targeted semantic and preservation failures, including the direct result and return path of each affected task. This is useful evidence of instruction application in one deliberately bounded case. It is source-only: no corrected UI was implemented, rendered, tested with users or aesthetically accepted. It does not establish reliable detection across arbitrary products or complete redesign quality.
