...
When existing obligations bundle independently closable outcomes, split them before recording partial success. Preserve the parent requirement, sources, covered outcomes, stage and consuming usages; link the replacement child IDs and account for every former clause. For example, “retain the default, save changes and restore the selection on return” can have separate checks for initialization, persistence and return context. A passed initialization check must not close the other two. Changed bindings require fresh execution or valid reuse under the ledger rules, never a copied blanket pass. Do not reduce the parent scope or create a new requirement for every test variation.

For a consequential workflow, write enough detail for another contributor to implement and review it without guessing the user's intent. Describe the actor and permitted resource scope, starting state and trigger, expected transition and visible result, state that must remain unchanged, and applicable cancellation, error and recovery behavior. Name relevant input/device, content, locale and theme conditions; link existing shared rules instead of duplicating them. Include a normal example and a meaningful boundary or failure example when applicable. Keep business definitions and unavailable facts unresolved rather than filling them with plausible assumptions.

An acceptance condition must distinguish success from failure: under a specified starting condition and action, what observable outcome must follow, what must not change, and how will it be checked? “The drawer works” is insufficient. “After cancelling an edited record, the stored values are unchanged and the same filtered row remains selected” gives an inspectable outcome; any focus or layout obligation can be linked separately. Specific visual criteria should refer to the agreed component rules and rendered conditions, not invented universal pixel values. Requirement detail should remove ambiguity, not add unrelated feature scope or repetition.
