...
Inventory the actual role records and their supported create, edit, assign, duplicate or delete actions before redesigning role management; include only actions the product supports. Readable permission domains are presentation groups, not replacement roles. Keep real role names, underlying permission keys and resource scopes traceable, including system-defined or externally managed restrictions.
...
Preserve observable widget anatomy, not just the label “statistics.” Compare the baseline and proposal with the same safe scenario: each simultaneous series or tier, its icon/label, current value, unit, history trace, threshold markers, status and linked action. A total, one generic sparkline or a static healthy badge does not replace several independently meaningful streams. Styling may change; removing or combining meaning is a separate decision requiring evidence and the applicable approval.

For existing live-event overlays, capture anchoring, event preview content, maximum visible stack, arrival order, overflow, dismissal, expiry, hover/focus behavior and retrieval destination. Exercise a burst with more than one item, not just an isolated alert. Preserve user controls and the distinction between transient preview, persistent history and acknowledgment; do not substitute a generic toast that loses event images or simultaneous events. Record any proposed placement or lifecycle change explicitly rather than assuming it follows the material style.
