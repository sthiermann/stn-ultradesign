...
Include applicable [layout integrity](layout-integrity.md) probes in each usage’s context plan. For analytical surfaces, distinguish widget purpose/duplication, presentation and [actual drilldown data outcomes](data-visualization.md#trace-selection-drill-and-return-as-separate-transitions) as separate obligations. Reuse these records rather than creating parallel inventories.
...
`not-applicable` means the agreed product/support contract does not require the behavior: an optional feature is genuinely absent, a state is unreachable by design, or a capability is unsupported. It requires `na_basis`, a reason and evidence tied to that contract. Record missing required behavior or a state made unreachable by a defect as `fail`. Missing credentials, tools, fixtures, time or authorization to perform a test are **blocked**, never a factual nonapplicability. An entity/context whose only checks are inapplicable stays unresolved: correct its model with evidence or record an approved exclusion, visibly changing full mode to scoped/sample mode.
