...
### Research the selected language for this project

Apply the same research depth to whichever direction the user chooses. A platform name, brand or adjective is a research brief, not a ready-made CSS preset. A product can inherit a company's identity without inheriting its navigation, and can reference a native system while remaining a web application.

1. **Define the reference.** Record the chosen identity/language, relevant products, device classes, version if specified, and inherited dimensions. Reuse confirmed answers. Ask only where different interpretations materially change the design; do not silently replace a requested generation with the newest one.
2. **Investigate primary evidence.** Find current official guidance, actual interface examples and documented component behavior relevant to the product's tasks. Inspect both appearances and relevant motion/states where required. Record text-only, image, video-frame, interaction and access limits separately. Do not confuse marketing artwork with application behavior.
3. **Extract relationships.** Identify navigation structure, hierarchy, typography, color roles, shape/inset relationships, materials, elevation, icons, movement and state semantics. Explain what each relationship does. Do not reduce a language to one color, rounded rectangles or a blur filter.
4. **Translate to this product.** Define tokens, composition and behavior within the existing feature/role contract. Record browser/native differences, licensed asset constraints, accessible alternatives and deliberate departures. Keep project-specific recipes in the project's brief/contract; do not turn them into generic skill defaults.
5. **Demonstrate the interpretation.** Before expanding an unsettled direction, render the primary work surface, a linked detail and a consequential control state using the same domain content as the strongest structural alternative. Keep their task consequences and selection reason together. If typography/proportion is open, compare the same title, values, field/error and action under plausible treatments within the authorized brand; record loaded fonts, wrapping and the smallest relevant layout. Do not reopen a settled choice merely to generate alternatives.
6. **Verify propagation and fidelity.** Use the per-usage map, actual rendering and separate review passes. Check the qualities chosen by the user, not just resemblance to a vendor label. Revisit the research when observed behavior or user feedback contradicts the interpretation. Report unsupported or unfinished cases rather than claiming the style is complete.

Use the [reference translation record](../assets/reference-translation.template.md) when a substantial reference-led direction needs a compact evidence trail. A polished proof helps select a direction; it neither completes the full usage audit nor authorizes feature loss or production implementation. A fresh project starts from its own requirements, not the previous project's selected language.
