...
A selected language is a research brief. Follow [project-research.md](project-research.md) for the authoritative study → translation → representative proof method, including applicability coverage, linked primary guidance, actual component-kit and interaction inspection, source conflicts, evidence limits and completion criteria. Apply it to the exact requested generation and platforms, in proportion to the affected tasks and families.

Carry this module's selective inheritance decisions into the study. Establish which relationships the user wants to retain in geometry, hierarchy, navigation, depth, material, color, typography, density or motion. Preserve confirmed preferences; resolve conflicting interpretations in the requirements register instead of silently combining them. Desktop, tablet and phone may require different expressions of the same direction.

Use the [reference translation record](../assets/reference-translation.template.md) in the target project's private working evidence. Link source observations and stable translation rules to the existing requirement IDs, component-family contracts, affected usages and acceptance checks. Keep project-specific references and recipes there; each project starts from its own brief.

After the representative proof, apply the [per-usage propagation method](#propagate-decisions-to-every-affected-usage) above. A proof helps settle the interpretation; it does not establish complete product coverage, retire features or grant implementation approval. Keep user assessment separate from technical verification, and leave a rejected direction open until the revised artifact is reviewed and the rejection is resolved. Use original implementation and authorized assets within the existing scope and approval process.
