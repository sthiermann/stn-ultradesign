...
Load for analytical screens, numerical widgets, maps, relationship diagrams, data tables and visual editors. These are operational design procedures synthesized for this skill. They are not a universal chart specification.
...
Validate the layers separately: the user’s problem, the data and task model, the encoding and interaction, and the rendering implementation. Faster rendering cannot prove that the represented task is useful.
...
A compact comparison chart can place a measure beside its target and meaningful reference ranges. Consider it when comparison is the task; do not replace every radial display by rule.
...
Use nonlinear scales only for a justified analytical purpose. Explain the transformation and ticks; define zero and negative values. Prefer aligned separate panels when an arbitrary dual axis would imply a misleading relationship.
...
Display uncertainty when omitting it would change the decision. Choose a treatment the audience can interpret and explain its meaning; do not add a decorative confidence band without an underlying calculation.
...
Small trend charts can sit beside related values and labels. Preserve enough measure, period and scale context to interpret them; compactness does not justify ambiguous comparisons.
...
Give a chart an accessible identity and concise account of its purpose and important patterns. Provide a discoverable description or structured data view appropriate to its complexity. Exact lookup in a table does not automatically explain trend, relationship or uncertainty.
...
Test the chosen library’s actual behavior. Labels on individual marks do not by themselves provide a coherent keyboard model or useful screen-reader sequence.
...
Distinguish row expansion, hierarchy navigation, filtered detail, sibling filtering and highlighting without exclusion. Make the interaction and scope visible. Hover decoration must not be the only route or clue to detail.
...
Use semantic HTML tables for tabular reading. Use an interactive grid when spreadsheet-like navigation or editing warrants its additional interaction model. Adding a grid role without the expected focus and keyboard behavior is insufficient.
...
Specify select-all scope separately from export and batch-action scope. Inspect the actual row model and configuration. A loaded subset is not “all results,” and a selected count may differ from the eligible count.
...
For editing, define start, commit, cancel, validation, pending save, conflict and recovery. Preserve recoverable drafts. Evaluate pagination and virtualization against keyboard movement, assistive technology, browser find, copy and export. Test whether expanded children remain visible or move to another page.
...
Make charts reusable through explicit data, scales, dimensions and interaction contracts. Verify the installed library version and manage lifecycle, updates and cleanup deliberately.
...
Verify installed versions, row models and server/API contracts before adopting implementation-specific behavior. A documented component capability does not establish the application’s configured behavior.
