...
For a redesign spanning sessions, multiple contributors or substantial feedback cycles, turn the existing requirements into a small set of executable work items in the project's established tracker or the [delivery record](../assets/delivery-plan.template.md). Use local records when no tracker is configured; creating public issues or sending notifications needs the relevant authorization. Keep private product evidence local unless sharing it is authorized. A focused repair can use one compact item. Tickets coordinate work; they do not replace discovery, requirements, the feature map or evidence.
...
Before assigning work, reconcile current state and dependencies, claim one owner per output boundary and select ready items. Parallelize independent items; serialize shared navigation/component decisions and the human interview. A waiting owner question does not block unrelated inspection. A worker receiving a stale contract must reconcile affected work before integration. On resumption, read the item and linked current decisions instead of relying on conversational memory.
