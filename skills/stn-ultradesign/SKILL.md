...
When work spans sessions, multiple contributors or substantial revision cycles, execute [bounded work items](references/delivery-workflow.md#execute-bounded-work-items) with dependencies, owners and evidence-based closure. Separate decisions, concept/implementation work and integrated verification. Default to local Markdown, reusing existing design records or an explicitly required tracker and requirement IDs. Keep one authoritative work state so unfinished obligations survive handoffs.

For a substantial concept, cover at least **twenty distinct, product-specific preference questions early**, and more whenever an unresolved decision materially affects the result, before committing to a visual direction. Carry forward confirmed answers; keep the remaining questions queued, with only one active question or agreed small batch. Reconcile its questions under the interview protocol before presenting another. Record explicit delegation if the user waives the remaining interview. Do not pad the count with discoverable facts, apply it to a narrow fix, or turn unanswered optional preferences into an approval gate. Use the [design brief](assets/design-brief.template.md) and distinguish confirmed answers, pending preferences and hypotheses.

During audits as well as concept work, follow the [interview protocol](references/discovery-and-preferences.md#keep-one-active-question-batch). Each material decision question visibly includes a recommendation, its reason, basis and tradeoff; when evidence cannot justify a choice, recommend the next way to resolve it. Distinguish applicable requirements, established patterns and taste. For factual questions, explain how the answer will be used without suggesting invented facts. Preserve the pending question across partial replies or presentation failures and continue independent inspection. An audit has no arbitrary question quota or ceiling.
...
- Before presenting a substantial concept as ready, walk its critical tasks from a visible entry through real proposed detail, outcome and return using [task-flow-design.md](references/task-flow-design.md). A feature-map entry, dead button or placeholder does not establish a working journey. Use the [presentation checkpoint](references/delivery-workflow.md#check-the-actual-review-package-before-presenting-it) to reconcile the running revision, review scope, capability destinations and evidence. Translate feedback into a concrete change and fresh rendered evidence; preserve the owner's rejection until resolved.
